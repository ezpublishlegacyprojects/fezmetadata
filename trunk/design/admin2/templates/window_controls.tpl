...
{def $node_url_alias      = $node.url_alias
     ...
     $valid_tabs = array( $default_tab, 'details', 'translations', 'locations', 'relations', 'states', 'metadata' )
}
...
<ul class="tabs">
    ...
    {* Metadata. *}
    <li id="node-tab-metadata" class="middle{if $node_tab_index|eq('metadata')} selected{/if}">
        <a href={concat( $node_url_alias, '/(tab)/metadata' )|ezurl} title="{'Hide object meta data overview.'|i18n( 'design/admin/node/view/full' )}">{'Meta Data'|i18n( 'design/admin/node/view/full' )}</a>
    </li>
