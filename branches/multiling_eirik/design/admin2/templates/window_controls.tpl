...
{def $node_url_alias      = $node.url_alias
     ...
     $meta_data_list 	  = fetch( 'fezmetadata', 'list_by_node_id', hash( node_id, $node.node_id ) )
     $meta_data_count	  = $meta_data_list|count
}
...
<ul class="tabs">
    ...
    {* Metadata *}
    {def }
    <li id="node-tab-meta" class="middle{if $node_tab_index|eq('meta')} selected{/if}">
        <a href={concat( $node_url_alias, '/(tab)/meta' )|ezurl}>{'Meta Data (%meta_count)'|i18n( 'design/admin/node/view/full', '', hash('%meta_count', $meta_data_count) )}</a>
    </li>
